...
public class GenericsUtils {
    ...
    /**
     * 通过反射,获得指定类的第一个接口的第一个泛型参数的实际类型. 如DaoSupport<Buyer>
     *
     * @param clazz 需要反射的类,该类必须继承泛型父类
     * @return 泛型参数的实际类型, 如果没有实现ParameterizedType接口，即不支持泛型，所以直接返回
     * <code>null</code>
     */
    public static Class<?> getInterfacesClassGenricType(Class<?> clazz) {
        return getInterfacesClassGenricType(clazz, 0, 0);
    }

    /**
     * 通过反射,获得指定类的接口的泛型参数的实际类型. 如DaoSupport<Buyer>
     *
     * @param clazz     需要反射的类,该类必须继承范型父类
     * @param intfIndex 接口类型所在索引,从0开始.
     * @param index     泛型参数所在索引,从0开始.
     * @return 范型参数的实际类型, 如果没有实现ParameterizedType接口，即不支持泛型，所以直接返回
     * <code>null</code>
     */
    public static Class<?> getInterfacesClassGenricType(Class<?> clazz, int intfIndex, int index) {
        Type[] genTypes = clazz.getGenericInterfaces();// 得到泛型父类
        if (genTypes == null || genTypes.length < 1) {
            return null;
        }
        if (intfIndex >= genTypes.length || intfIndex < 0) {
            throw new RuntimeException("你提供的接口索引" + (intfIndex < 0 ? "不能小于0" : "超出了接口的总数"));
        }
        Type genType = genTypes[intfIndex];
