...
public class Document {
    ...
    private static final String ELE_SEPARATOR = "/";
    ...
    public Document(String xmlString) throws IllegalArgumentException {
        if (xmlString == null || xmlString.length() == 0)
            throw new IllegalArgumentException(
                    "Argument xmlString cannot null!");
     ...
     * @return String[]
     */
    public String getElementSingleByTag(String tagName) {

        String[] arr = getElementsByTag(tagName);
    ...
    public String getElementBySinglePath(String singlePath) {
        String[] path = singlePath.split(ELE_SEPARATOR);
    ...
    /**
     * 用xpath模式提取元素从多重元素中获取指批定元素,以/为分隔符 元素后无索引序号则默认为0,例如: ROOT/PARENT[2]/CHILD[1]
     *
     * @param singlePath String
     * @return String
     */
...
//					System.out.println(input + "----" + tagName);
    ...
    public static void main(String[] args) {
        String xml = "<ROOT>sss <PARENT><name />sss <CHILD>aaaa</CHILD>ss </PARENT>sss </ROOT>";
        String child = new Document(xml).getElementByMultiPath("ROOT[0]/PARENT/CHILD");
        System.out.println(child);

        String value = Document.getElementText(child);
        System.out.println("value:" + value);

        String tags[] = new Document(xml).getElementsByTag("CHILD");

        for (String tag : tags) {
            System.out.println("tag:" + tag);
            System.out.println("value:" + Document.getElementText(tag));
        }
    }
