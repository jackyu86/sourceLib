...
module.exports = function ($scope, $rootScope, $http, $stateParams, $state, Upload,
                           $mdDialog, $mdToast, $element, $location, $anchorScroll) {
    $scope.data = {};
    $scope.init = function () {
        $scope.categories = [];
    ...
    $scope.init();
    $http.get("/admin/ajax/product/" + $stateParams.id)
        .then(function (response) {
            $scope.data.product = response.data;
            if ($scope.data.product.insuranceCategoryIds && $scope.data.product.insuranceCategoryIds.length > 0) {
                $scope.initProductCategories();
            }
            if ($scope.data.product.jobIds && $scope.data.product.jobIds.length > 0) {
                $scope.initProductJobs();
            }
            $scope.data.product.period = $scope.initPeriod($scope.data.product.period);
            $scope.data.product.payment = $scope.initPayment($scope.data.product.payment);
            $scope.initJobTree();
            $scope.initInsurances();
            if ($scope.data.product.insuranceDividendTypes == null) {
                $scope.data.product.insuranceDividendTypes = [];
            }
            $scope.initCities();
            $scope.initAreas();
            $scope.initInsuranceClause();
            $scope.initPlp();
            $scope.initPdp();
            $scope.initFormGroups();
            $scope.initInsuranceClaims();
        });
    $scope.initPeriod = function (period) {
        if (period.ages == null) {
            period.ages = [];
        }
        if (period.earliestInsuranceTime == null) {
            period.earliestInsuranceTime = {displayName: "1天", value: 1, unit: "DAY"};
        }
        if (period.fixedValue == null) {
            period.fixedValue = {unit: "DAY"};
        }
        if (period.inputMax == null) {
            period.inputMax = {displayName: "1天", value: 1, unit: "DAY"};
        }
        if (period.inputMin == null) {
            period.inputMin = {displayName: "1天", value: 1, unit: "DAY"};
        }
        if (period.inputUnit == null) {
            period.inputUnit = "DAY";
        }
        if (period.selections == null) {
            period.selections = [];
        }
        if (period.startTimeType == null) {
            period.startTimeType = "USER_INPUT";
        }
        return period;
    };
    $scope.initPayment = function (payment) {
        if (payment == null) {
            return {
                methods: [],
                fixedPeriods: [],
        ...
        if (payment.methods == null) {
            payment.method = [];
        }
        if (payment.fixedPeriods == null) {
            payment.fixedPeriods = [];
        }
        if (payment.irregularPeriods == null) {
            payment.irregularPeriods = [];
        }
    ...
    $scope.initJobTree = function () {
        if ($scope.data.product.jobIds == null) {
            $scope.data.product.jobIds = [];
        }
        if ($scope.data.product.jobLevels == null) {
            $scope.data.product.jobLevels = [];
        }
        $http.put("/admin/ajax/job-tree/find", $scope.jobTreeQuery)
            .then(function (response) {
                $scope.jobTrees = response.data.items;
                for (var i = 0; i < $scope.jobTrees.length; i++) {
                    if ($scope.jobTrees[i].id == $scope.data.product.insuranceJobTreeId) {
                        $scope.jobTree = $scope.jobTrees[i];
                        break;
                    }
                }
                if ($scope.data.product.insuranceJobTreeId == null || !$scope.data.product.insuranceJobTreeId)return;
                $http.get("/admin/ajax/job/tree/" + $scope.data.product.insuranceJobTreeId)
                    .then(function (response) {
    ...
    $scope.initInsurances = function () {
        if ($scope.data.product.insurances == null) {
            $scope.data.product.insurances = [];
        }
        //$scope.insurances = angular.copy($scope.data.product.insurances);
        for (var i = 0; i < $scope.data.product.insurances.length; i++) {
            $scope.insuranceIds.push($scope.data.product.insurances[i].insuranceId);
            $scope.getInsurance(i);
        }
    };
    $scope.getInsurance = function (index) {
        var insurance = $scope.data.product.insurances[index];
        $http.get("/admin/ajax/insurance/" + insurance.insuranceId)
            .then(function (response) {
                $scope.data.product.insurances[index].name = response.data.name;
                $scope.getLiabilities(index);
            })
    };
    $scope.getLiabilities = function (index) {
        var liabilities = $scope.data.product.insurances[index].liabilities;
        for (var i = 0; i < liabilities.length; i++) {
            $scope.getLiability(index, i);
        }
    };
    $scope.getLiability = function (index, indexLiability) {
        $http.get("/admin/ajax/insurance/liability/" + $scope.data.product.insurances[index].liabilities[indexLiability].insuranceLiabilityId)
            .then(function (response) {
                $scope.data.product.insurances[index].liabilities[indexLiability].name = response.data.name;
                $scope.liabilityGroupFieldNames.push(response.data.id);
            });
    };
    $scope.initCities = function () {
        if ($scope.data.product.insuranceCityIds == null) {
            ...
            var req = {};
            req.cityIds = angular.copy($scope.data.product.insuranceCityIds);
            $http.put("/admin/ajax/city/batch", req)
                .then(function (response) {
                    var temp = [];
                    for (var i = 0; i < response.data.length; i++) {
                        var c = response.data[i];
                        if (temp.indexOf(c.provinceId) < 0) {
                            temp.push(c.provinceId);
                        }
                    }
                    $http.get("/admin/ajax/province")
                        .then(function (response) {
    ...
    $scope.getCities = function (i) {
        $http.get("/admin/ajax/province/" + $scope.provinces[i].id)
            .then(function (response) {
                ...
                var index = $scope.selectedProvinceIds.indexOf($scope.provinces[i].id);
                if (index > -1) {
                    $scope.productProvinces[index].cities = response.data;
                }
            });
    };
    ...
    $scope.initAreas = function () {
        if ($scope.data.product.insuranceAreaIds == null) {
    ...
    $scope.initInsuranceClause = function () {
        if ($scope.data.product.insuranceClauseIds == null) {
            $scope.data.product.insuranceClauseIds = [];
        }
        var req = {};
        req.ids = angular.copy($scope.data.product.insuranceClauseIds);
        $http.put("/admin/ajax/clause/batch", req)
            .then(function (response) {
                $scope.productClauses = response.data;
            });
    };
    $scope.initPlp = function () {
        if ($scope.data.product.plp == null) {
            $scope.data.product.plp = [];
        }
        var plp = $scope.data.product.plp;
        for (var i = 0; i < plp.length; i++) {
            $scope.getListFormFieldDisplayName(i);
        }
    };
    $scope.getListFormFieldDisplayName = function (i) {
        var f = $scope.data.product.plp[i];
        $http.get("/admin/ajax/form-group/name/" + f.group)
            .then(function (resposne) {
                f.groupDisplayName = resposne.data.displayName;
            });
        if (f.group == 'liability') {
            $http.get("/admin/ajax/insurance/liability/" + f.field)
                .then(function (response) {
                    f.fieldDisplayName = response.data.name;
                })
        } else if (f.group == 'product' && f.field == "serial") {
            f.fieldDisplayName = '系列';
        } else {
            $http.get("/admin/ajax/form-field/group/" + f.group + "/field/" + f.field)
                .then(function (response) {
                    f.fieldDisplayName = response.data.displayName;
                });
        }
    };
    $scope.initPdp = function () {
        if ($scope.data.product.pdp == null) {
            $scope.data.product.pdp = [{group: "product", field: "serial", editable: false}];
        }
    };
    $scope.initFormGroups = function () {
        ...
        if ($scope.data.product.formGroups == null) {
            $scope.data.product.formGroups = [];
        }
        for (var i = 0; i < $scope.data.product.formGroups.length; i++) {
            $scope.getFormGroupName(i);
        }
    };
    $scope.initFormFieldPdp = function (i, j) {
        var g = $scope.data.product.formGroups[i];
        var f = g.fields[j];
        var k;
        var p;
        var list = $scope.data.product.pdp;
        for (k = 0; k < list.length; k++) {
            p = list[k];
            if (p.group == g.name && p.field == f.name) {
                $scope.data.product.formGroups[i].fields[j].pdp = true;
                $scope.data.product.formGroups[i].fields[j].pdpEditable = p.editable;
            }
        }
        list = $scope.data.product.plp;
        for (k = 0; k < list.length; k++) {
            p = list[k];
            if (p.group == g.name && p.field == f.name) {
                $scope.data.product.formGroups[i].fields[j].plp = true;
                $scope.data.product.formGroups[i].fields[j].plpEditable = p.editable;
            }
        }
    };
    ...
    $scope.getFormGroupName = function (i) {
        var g = $scope.data.product.formGroups[i];
        $http.get("/admin/ajax/form-group/name/" + g.name)
            .then(function (response) {
                $scope.data.product.formGroups[i].displayName = response.data.displayName;
                $scope.data.product.formGroups[i].id = response.data.id;
                $scope.data.product.formGroups[i].selectedFieldNames = [];
                $scope.data.product.formGroups[i].formFieldQuery = {page: 1, limit: 999999, displayName: null, order: ""};
                $scope.selectedFormGroups.push($scope.data.product.formGroups[i].name);
                for (var j = 0; j < g.fields.length; j++) {
                    $scope.data.product.formGroups[i].selectedFieldNames.push($scope.data.product.formGroups[i].fields[j].name);
                    $scope.getFormFieldName(i, j);
                    $scope.initFormFieldPdp(i, j);
                }
            })
    };
    $scope.getFormFieldName = function (i, j) {
        var g = $scope.data.product.formGroups[i];
        var f = g.fields[j];
        if (g.name == 'liability') {
            $http.get("/admin/ajax/insurance/liability/" + f.name)
                .then(function (response) {
                    $scope.data.product.formGroups[i].fields[j].displayName = response.data.name;
                });
        } else if (g.name == 'product' && f.name == 'serial') {
            $scope.data.product.formGroups[i].fields[j].displayName = '系列';
        } else {
            $http.get("/admin/ajax/form-field/group/" + g.name + "/field/" + f.name)
                .then(function (response) {
                    $scope.data.product.formGroups[i].fields[j].displayName = response.data.displayName;
                    $scope.data.product.formGroups[i].fields[j].type = response.data.type;
                });
        }
        if ($scope.data.product.formGroups[i].fields[j].options == null) {
            $scope.data.product.formGroups[i].fields[j].options = {};
        }
    };
    $scope.initInsuranceClaims = function () {
        if ($scope.data.product.insuranceClaimIds == null) {
            $scope.data.product.insuranceClaims = [];
        } else {
            $http.put("/admin/ajax/insurance/claim/batch", {ids: $scope.data.product.insuranceClaimIds})
                .then(function (response) {
                    $scope.productClaims = response.data;
                })
        }
    };
    ...
    $scope.productStatus = [
        {name: "上架", value: "ACTIVE"},
        {name: "下架", value: "INACTIVE"}
    ];
    ...
    $scope.initProductCategories = function () {
        var request = {categoryIds: $scope.data.product.insuranceCategoryIds};
        $http.put("/admin/ajax/insurance/category/batch/find", request)
            .then(function (response) {
                $scope.productCategories = response.data;
            })
    };
    $scope.initProductJobs = function () {
        var request = {jobIds: $scope.data.product.jobIds};
        $http.put("/admin/ajax/job/batch/find", request)
            .then(function (response) {
                $scope.productJobs = response.data;
            });
    };
    ...
    $scope.getSerial = function () {
        ...
        $http.put("/admin/ajax/product/serial/find", $scope.serialQuery)
            .then(function (response) {
                ...
                $scope.serialList.splice(0, 0, {id: null, name: "无"});
            });
    };
    ...
    $scope.addClaim = function () {
        $scope.data.product.insuranceClaims.push({});
    };
    $scope.deleteClaim = function (index) {
        $scope.data.product.insuranceClaims.splice(index, 1);
    };
    ...
    $scope.toggleField = function (field, group) {
        ...
        if (index > -1) {
            ...
            $scope.selectedElement = null;
            console.log($scope.data.product.formGroups[gIndex]);
        } else {
    ...
    $scope.save = function () {
        if (!$scope.check($scope.data.product))return false;
        var min = 99;
        var max = 0;
        for (var i = 0; i < $scope.jobs.length; i++) {
        ...
        $http.post("/admin/ajax/product/audit", req)
            .then(function (response) {
                $scope.toast("提交更新请求成功");
            }).catch(function (response) {
            if (response.status == 500) {
                var error = response.data.message;
                $scope.toast("更新失败!原因：" + error);
                if (error == "当前版本已过时") {
                    $state.go("console.product.list");
                }
            } else {
