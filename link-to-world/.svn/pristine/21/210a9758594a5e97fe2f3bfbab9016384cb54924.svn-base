...
module.exports = function ($scope, $rootScope, $http, $stateParams, $state, Upload, $mdDialog, $mdToast) {
    ...
    $http.get("/admin/ajax/insurance/" + $stateParams.id)
        .then(function (response) {
            $scope.data.insurance = response.data;
            console.log(response.data);
            for (var i = 0; i < $scope.data.insurance.liabilities.length; i++) {
                $scope.getLiability(i);
            }
            $scope.selectedXFactors = $scope.data.insurance.priceTable.xFactors;
            $scope.data.table = angular.toJson($scope.data.insurance.priceTable.table);
        });
    $scope.getLiability = function (i) {
        var l = $scope.data.insurance.liabilities[i];
        $http.get("/admin/ajax/insurance/liability/" + l.insuranceLiabilityId)
            .then(function (response) {
                $scope.data.insurance.liabilities[i].liabilityName = response.data.name;
                $scope.data.insurance.liabilities[i].toggle = true;
            })
    };
    ...
    $scope.save = function () {
        ...
        $http.put("/admin/ajax/insurance/update/" + $stateParams.id, $scope.data.insurance)
            .then(function (response) {
                $scope.toast("保存成功");
