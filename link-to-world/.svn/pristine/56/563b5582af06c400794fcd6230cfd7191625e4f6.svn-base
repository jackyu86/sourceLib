 ...
 * @author miller
 */
public class DeliverTypeFieldType extends AbstractFieldType<String> {
    @Inject
    EnumDeliverTypeWebServiceClient enumDeliverTypeWebService;

    public DeliverTypeFieldType() {
        super(String.class);
    }
    ...
    public boolean validate(Object value, Context context) {
        ...
        enumDeliverTypeWebService.findAll().list
            .forEach(enumDeliverTypeResponse -> {
                EnumConstant enumConstant = new EnumConstant();
                enumConstant.displayName = enumDeliverTypeResponse.name;
                enumConstant.value = enumDeliverTypeResponse.value;
