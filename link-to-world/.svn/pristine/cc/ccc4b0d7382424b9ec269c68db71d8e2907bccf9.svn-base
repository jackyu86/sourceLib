 ...
 * @author miller
 */
public class InvoiceDeliveryTypeFieldType extends AbstractFieldType<String> {
    @Inject
    EnumInvoiceDeliverTypeWebServiceClient enumInvoiceDeliverTypeWebService;

    public InvoiceDeliveryTypeFieldType() {
        super(String.class);
    }
    ...
    public boolean validate(Object value, Context context) {
        ...
        enumInvoiceDeliverTypeWebService.findAll().list
            .forEach(enumInvoiceDeliverTypeResponse -> {
                EnumConstant enumConstant = new EnumConstant();
                enumConstant.displayName = enumInvoiceDeliverTypeResponse.name;
                enumConstant.value = enumInvoiceDeliverTypeResponse.value;
