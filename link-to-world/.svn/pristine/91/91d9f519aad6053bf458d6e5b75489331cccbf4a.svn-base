 ...
 * @author chi
 */
public class JDBCAddressServiceImpl implements AddressService {
    @Inject
    JDBCRepository<Address> repository;

    @Override
    public Address get(String addressId) {
        Optional<Address> address = repository.query("id=?", addressId).findOne();
